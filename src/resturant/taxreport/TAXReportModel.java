 ...
 * @author SUSHIL
 */
public class TAXReportModel extends DBConnect {
     public DefaultTableModel getTAXList(Date[] date){
          PreparedStatement  stmtget;
          ResultSet rsGet;
          ArrayList<Object[]> data = new ArrayList<Object[]>();
           Object[][] finaldata = null;
           String[] ColumnNames = new String[]{"Bill Id","Amount","TAX Amount","TAX Percentage(%)"}; 
           String strQuery = "SELECT bill.bill_id,bill.item_total_amount,bill.vat FROM bill WHERE vat >= 0 AND bill.bill_datetime >= ? AND bill.bill_datetime <= ?";
...
//              stmtget.setTimestamp(2, new Timestamp(date[1].getTime()));
              rsGet =  stmtget.executeQuery();
               while(rsGet.next()){
                   BigDecimal SVCPer = rsGet.getBigDecimal("vat").divide(rsGet.getBigDecimal("item_total_amount")).multiply(BigDecimal.valueOf(100));
                 Object[] row = new Object[]{rsGet.getInt("bill_id"),rsGet.getBigDecimal("item_total_amount").setScale(2, RoundingMode.HALF_UP),rsGet.getBigDecimal("vat").setScale(2, RoundingMode.HALF_UP),SVCPer};
//                 for(Object rw:row){
...
//                 }
               ...
               JOptionPane.showMessageDialog(null, se+"From getTAXList");
           }
           finally{
